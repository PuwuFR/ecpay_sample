...
namespace ecpay_sample.ST
{
    public class STPay
    {
        string Domain = "http://35.227.82.89/Ecpay";
        public string GoCC(int Amt, int CustomerId)
        {

            string TransactionId = (long.Parse(DateTime.Now.ToString("yyyyMMddHHmmss")) + 55688).ToString(); 

            string formhtml = this.CheckOut(PaymentMethod.Credit, (Domain + "/Paid"), (Domain), TransactionId, Amt);
